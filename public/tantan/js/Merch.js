...
Orders.prototype.purchaseUnifiedOrder = function(){
	...
	if(localStorage.tantanXieYi != true && localStorage.tantanXieYi != 'true'){
		var xieYi = new XieYi()
		xieYi.render();
		return;
	}
...
Orders.prototype.checkWinningInfo = function(){
	...
	if(DEBUG){
		that.orderId='MQ14020180817095915524122049'
	}
	...
	if(that.winningFlag){
		...
		for (var i = 0; i < GridItemArr.length; i++) {
			if(that.duangProductVo.id == GridItemArr[i].id){
				//确定出口
				var _index = GridItemArr[i].bgImgSrc.index;

				for(var x in  Map.EggImg){
					if( Map.EggImg[x].imgIndex == _index){
						animation.winnerIndex = Map.EggImg[x].index;
					}
				}
		...
		animation.winnerIndex = Map.getEmptyIndex();
	}
